...
function getRegExePath () {
  if (process.platform === 'win32') {
    let proc = process
    return path.join(proc.env.windir, 'system32', 'reg.exe')
  } else {
    return 'REG'
  }
}
...
function RegistryItem (host, hive, key, name, type, value, arch) {
  ...
  var _hive = hive   // registry hive
  var _key = key    // registry key
  var _name = name   // property name
  var _type = type   // property type
  var _value = value  // property value
  var _arch = arch   // hive architecture
...
function Registry (options) {
  ...
  var _host = '' + (_options.host || '')    // hostname
  var _hive = '' + (_options.hive || HKLM)  // registry hive
  var _key = '' + (_options.key || '')    // registry key
  var _arch = _options.arch || null         // hive architecture
...
Registry.prototype.values = function values (cb) {
  ...
  var buffer = ''
  var self = this
  var error = null // null means no error previously reported.
  ...
  proc.on('close', function (code) {
    if (error) {
      ...
      for (var j = 0, len = items.length; j < len; j++) {
        var match = ITEM_PATTERN.exec(items[j])
        var name
        var type
        var value
...
Registry.prototype.get = function get (name, cb) {
  ...
  var args = ['QUERY', this.path]
  if (name === '') { args.push('/ve') } else { args = args.concat(['/v', name]) }
