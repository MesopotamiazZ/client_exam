  ...
  <div class="right-lev3-describe" v-if='showLev3Number'>
    ...
    <div class="lev3-right-content">
      ...
      <div class="right-table-des-con paddingL20 paddingR20">
        <div class="table-des-con-out paddingT15">
          <div class="discribTab">
            ...
            <div class="discribTableTab">
              <q-table
                ref='discribBaseTable'
                :key="'level3-table-discrib'"
        ...
        <div class="right-table-des-last flex paddingT15 paddingR20 paddingB20">
          <p class="marginR20">最后修改人：<span>{{lev3PublicData.operater}}</span></p>
...
export default {
  ...
  data () {
    return {
      ...
      is_default_res: 0, // 是否默认
      discribCanChangeBtn: true, // 选择按钮是否可点击
      ids: [],
      describeType: {
        sort: 0,
  ...
  watch: {
    delData (val) {
      if (val.length === 0 && this.describeType.isSelectMode) {
        this.$refs.discribBaseTable.allSelection = []
      }
    },
    ...
    data: {
      async handler(val) {
        ...
        // console.log(val.pe_res_items)
        // console.log(this.describeType.data)
        // this.describeType.sort = this.describeType.data.length // 总的sort。主要在新增使用
  ...
  methods: {
    ...
    reqLev3Des () { // 表格请求
      this.$nextTick(async () => {
        ...
        await this.$refs.discribBaseTable.request()
      // await this.getRightSingleDes()
        // console.log(this.singleRightDataDes)
        console.log(this.singleRightDataDes)
    ...
    editSinglePro () { // 编辑单个项目
      // console.log('编辑单个项目')
      // console.log(this.lev3PublicData)
      this.$emit('edit-single-pro-des', this.lev3PublicData)
      // this.$emit('edit-single-pro-des', this.lev3PublicData.id, this.lev3PublicData)
    },
    delSinglePro () { // 删除单个项目
      // console.log('删除单个项目')
      this.$emit('del-single-pro')
    },
    discribTabAdd () { // 新增描述型三级数据
      // console.log('新增描述型三级数据')
      this.$emit('add-describ')
    },
    delDiscribBtn () { // 头部删除三级数据
      // console.log('头部删除三级数据')
      this.$emit('del-describ', this.delData)
    },
    delDiscribRow (data) { // 删除描述型
      // console.log('删除描述型')
      this.$emit('del-describ', data)
    },
    editDiscribRow (data) { // 编辑描述型
      // console.log('编辑描述型')
