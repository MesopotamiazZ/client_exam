...
*/
<template>
  <div class="edit-single-pro-modal" v-if="ifShowModal">
    <Modal v-model="ifShowModal" 
      ...
      :closable="false">
      <div slot="header">
        <alert-header :headerText='"编辑单个项目"' @alert-retrun-num='editSort' :headerVal='modalForm.data.sort'></alert-header>
      </div>
      ...
      <div class="aspm-main">
        <div class="partOne">
          <Form
            ...
            <FormItem prop="type_id" label="项目类别" class="modal-form-row three-row" id="three-row-left">
              <Select 
                v-model="modalForm.data.type_id" 
                filterable 
                :class='{selectVerificationTs: isEmpty(modalForm.data.type_id)}'
                placeholder=''
                @on-change='typeIdChange'>
                <Option v-for="item in selectTypeId" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>
            </FormItem>
            <FormItem prop="res_type" label="结果类型" class="modal-form-row three-row" id="three-row-right">
              <Select 
            ...
            <FormItem prop="sex" label="适用性别" class="modal-form-row four-row" id="four-row-left">
              <Select 
                v-model="modalForm.data.sex" 
                filterable 
                placeholder='' 
                :class='{selectVerificationTs: isEmpty(modalForm.data.sex)}'>
            ...
            <div class="partTwo" v-if="!isNumberResultLoc">
              <!-- <Form
                ...
                <FormItem prop="low_limit_man" label="参考下限（男）" class="modal-form-row second-row">
                  <z-input type="number" :disabled='ifMan' v-model.trim="modalForm.data.low_limit_man"></z-input>
                </FormItem>
                <FormItem prop="upper_limit_man" style="margin-left: 30px;" label="参考上限（男）" class="modal-form-row second-row">
            ...
            <div class="partThree">
              <!-- <Form
                ...
                <Checkbox class="marginR20" style="line-height: 30px;margin-bottom: 20px;" v-model="modalForm.data.is_has_summary" :true-value=1 :false-value=0>
                  是否小结
                </Checkbox>
                <FormItem prop="" label="备注" class="modal-form-row three-row" id="remark">
                  <Input type="textarea" v-model.trim="modalForm.data.remark" :rows="2"></Input>
                </FormItem>
                <FormItem prop="" class="lastPerson">
                  <div class="flex">
                    <p class="marginR20">最后修改人：<span>{{modalForm.data.operater}}</span></p>
                    <p class="flx1">最后修改时间：<span>{{modalForm.data.updatedAt}}</span></p>
                  </div>
                </FormItem>
              <!-- </Form> -->
            </div>
...
export default {
  ...
  data () {
    ...
    return {
      isNumberResultLoc: false, // 是否显示数值型
      resTypeDis: 1, // 描述型
      resTypeNum: 2, // 数值型
      ifMan: true, // 参考男是否能编辑
  ...
  methods: {
    ...
    typeIdChange (val) {
      // console.log(val)
      this.modalForm.data.type_id = val
      // console.log(this.modalForm.data.type_id)
    },
    ...
    async singleModalSave (name) { // 修改保存
      if (await this.$refs[name].validate()) {
        this.$emit('single-modal-save', this.modalForm.data.id, this.modalForm.data)
      }
    },
  },
  watch: {
    ...
    data: {
      handler(val) {
        ...
        // console.log(val)
        // console.log(typeof val.min_val)
        val.low_limit_man = val.low_limit_man ? parseFloat(val.low_limit_man) : null
        val.upper_limit_man = val.upper_limit_man ? parseFloat(val.upper_limit_man) : null
        val.low_limit_woman = val.low_limit_woman ? parseFloat(val.low_limit_woman) : null
        val.upper_limit_woman = val.upper_limit_woman ? parseFloat(val.upper_limit_woman) : null
        // val.max_val = val.max_val === undefined ? parseFloat(val.max_val) : null
        // val.min_val = val.min_val === undefined ? parseFloat(val.min_val) : null
        val.max_val = val.max_val ? parseFloat(val.max_val) : 0
        val.min_val = val.min_val ? parseFloat(val.min_val) : 0
        this.modalForm.data = _.clone(val)
        // console.log(this.modalForm.data)
        if (val.report_name === null) {
          this.modalForm.data.report_name = ''
        }
        // console.log(val.brief_summary)
        // console.log(this.modalForm.data.brief_summary)
        // console.log(val.sex)
        this.modalForm.data.updatedAt = timeFilter(val.updated_at)
        this.modalForm.data.type_id = val.type ? val.type.id : null
        this.modalForm.data.type_id = val.type.id
        if (val.reference_rule === null) {
          this.modalForm.data.reference_rule = {
            left: null,
            right: null,
          }
        } else {
          this.modalForm.data.reference_rule = _.clone(val.reference_rule)
        }
      },
      deep: true,
    },
