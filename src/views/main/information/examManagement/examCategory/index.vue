/**
 * Created 张祥
 * Date: 2018/01/19
 * content:体检类别管理
 * updated：2018/01/26
 */
<template>
  <div class="exam-category-wrapper">
    <!-- 主体 -->
    <div class="exam-category">
      <div class="header-wrapper">
        <div class="table-header">
          <title-has-slot>
            <span slot="titletHasSlotLeftTitle">体检类别列表</span>
            <div slot="titletHasSlotRightContent" class="titlet-slot-left">
              <Button type="primary" v-if="selectMode" class="fr " @click="selectMode=false" key="1">取 消</Button>
              <!-- <Button type="ghost" v-if="selectMode" class="fr marginR10" @click="print" key="2">打 印</Button> -->
              <Button
                key="3"
                type="success"
                v-if="selectMode"
                class="fr marginR10  delete-button"
                :disabled="selection.length===0"
                @click="deleteAllSelect()">删 除
              </Button>
              <Button type="ghost" v-if="!selectMode" class="fr " @click="selectMode=true" key="4">选 择</Button>
              <Button type="ghost" v-if="!selectMode" class="fr marginR10" @click="exportBtn" key="5">导 出</Button>
              <Button type="ghost" v-if="!selectMode" class="fr marginR10" @click="add" key="6">新 增</Button>
              <search-input
                :searchWidth="220"
                class="fr marginL20 marginR20"
                searText="请输入搜索内容"
                @on-enter="search"
                @on-Icon="search"
                @on-cancel="search">
              </search-input>
            </div>
          </title-has-slot>
        </div>
        <div class="contently" ref="contentTable">
          <q-table
            ref="zBaseInfoTable"
            :isSelectMode="selectMode"
            :params="params"
            :ifEdit="true"
            :columns="columns"
            :selectMode="selectMode"
            :data="examCategoryList.items"
            :requestDataApi="getExamCategoryList"
            :height="tableHeight"
            :pageInfo="pageInfo"
            @after-request="afterRequest"
            @delete-row="delOne"
            @delete="delOne"
            @edit-row="editOne"
            @edit="editOne"
            @on-row-dbClick="editOne"
            @getRequestParams="getRequestParams"
            @all-selection="selectChange">
          </q-table>
        </div>
      </div>
    </div>
    <!-- 主体结束 -->

    <!-- 新增 -->
    <div class="add-modal" v-if="addModal">
      <Modal
        v-model="addModal"
        width="381"
        :transfer="false"
        :closable="false"
        :mask-closable="false"
        :transition-names="['']">
        <div slot="header">
          <alert-header
            headerText="新增体检类别"
            :headerVal="total+1"
            @alert-retrun-num="getSort">
          </alert-header>
        </div>
        <div class="body">
          <Form ref="formAdd"
            :model="formAdd"
            :rules="rulesFormAdd">
            <div class="row clearfix">
              <span class="textJustify fl lineH30" style="width:72px;">体检类型</span>
              <FormItem prop="pe_type" class="fr">
                <Select
                  filterable
                  v-model="formAdd.pe_type"
                  style="width:256px"
                  placeholder=""
                  :class="{selectionRequired:isEmpty(formAdd.pe_type)}">
                  <Option v-for="item in pe_typeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                </Select>
              </FormItem>
            </div>
            <div class="row clearfix">
              <span class="textJustify fl lineH30" style="width:72px;">体检类别名称</span>
              <FormItem prop="name" class="fr">
                <Input style="width:256px" :class='{inputRequired: isEmpty(formAdd.name)}' v-model.trim="formAdd.name"></Input>
              </FormItem>
            </div>
            <div class="row clearfix">
              <span class="textJustify fl lineH30" style="width:72px;margin-top:3px;">备注</span>
              <FormItem class="fr marginB15">
                <Input style="width:256px" v-model="formAdd.remark" type="textarea" :maxlength="255" :row="2"></Input>
              </FormItem>
            </div>
            <div class="row check-group-row clearfix">
              <div class="item fr">
                <Checkbox v-model="formAdd.status" true-value=1 false-value=0>启用</Checkbox>
              </div>
              <div class="item fr" :style="{marginRight:'30px'}">
                <Checkbox v-model="formAdd.is_upload" true-value=1 false-value=0>是否上传</Checkbox>
              </div>
              <div class="item fr" :style="{marginRight:'30px'}">
                <Checkbox v-model="formAdd.is_print" true-value=1 false-value=0>是否打印证明</Checkbox>
              </div>
            </div>
          </Form>
        </div>
        <div slot="footer" class="clearfix">
          <Button type="ghost" class="fr marginR20 marginB20" @click="addModal = false">取 消</Button>
          <Button type="primary" class="fr marginR20 marginB20" @click="submitAdd()">保 存</Button>
        </div>
      </Modal>
    </div>
    <!-- 新增结束 -->

    <!-- 编辑 -->
    <div class="add-modal" v-if="editModal">
      <Modal
        v-model="editModal"
        width="381"
        :transfer="false"
        :closable="false"
        :mask-closable="false"
        :transition-names="['']">
        <div slot="header">
          <alert-header
            headerText="编辑体检类别"
            :headerVal="formAdd.sort"
            @alert-retrun-num='getSort'>
          </alert-header>
        </div>
        <div class="body">
          <Form ref="formCustomEdit"
          :style="{marginBottom:'0'}"
            :model="formAdd"
            :rules="rulesFormAdd">
            <div class="row clearfix"> 
              <span class="textJustify fl lineH30" style="width:72px;">体检类型</span>
              <FormItem prop="pe_type" class="fr">
                <Select
                  filterable
                  v-model="formAdd.pe_type"
                  style="width:256px"
                  placeholder=""
                  :class="{selectionRequired:isEmpty(formAdd.pe_type)}">
                  <Option v-for="item in pe_typeList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                </Select>
              </FormItem>
            </div>
            <div class="row clearfix">
              <span class="textJustify fl lineH30" style="width:72px;">体检类别名称</span>
              <FormItem prop="name" class="fr">
                <Input style="width:256px" :class='{inputRequired: isEmpty(formAdd.name)}' v-model.trim="formAdd.name"></Input>
              </FormItem>
            </div>
            <div class="row clearfix">
              <span class="textJustify fl lineH30" style="width:72px;margin-top:3px;">备注</span>
              <FormItem class="fr marginB15">
                <Input style="width:256px" v-model="formAdd.remark" type="textarea" :maxlength="255" :row="2"></Input>
              </FormItem>
            </div>
            <div class="row check-group-row clearfix">
              <div class="item fr">
                <Checkbox v-model="formAdd.status" true-value=1 false-value=0>启用</Checkbox>
              </div>
              <div class="item fr" :style="{marginRight:'30px'}">
                <Checkbox v-model="formAdd.is_upload" true-value=1 false-value=0>是否上传</Checkbox>
              </div>
              <div class="item fr" :style="{marginRight:'30px'}">
                <Checkbox v-model="formAdd.is_print" true-value=1 false-value=0>是否打印证明</Checkbox>
              </div>
            </div>
            <div class="row clearfix lastOperater">
              <span class="textJustify fl marginR20">最后修改人：{{curRow?curRow.operater:''}}</span>
              <span class="textJustify fl">最后修改时间：{{updated_at}}</span>
            </div>
          </Form>
        </div>
        <div slot="footer" class="clearfix">
          <Button type="ghost" class="fr marginR20 marginB20" @click="editModal=false">取 消</Button>
          <Button type="primary" class="fr marginR20 marginB20" @click="submitEdit()">保 存</Button>
        </div>
      </Modal>
    </div>
    <!-- 编辑结束-->

    <!-- 删除多个弹窗 -->
    <div class="del-disease modal">
      <affirm-del-modal :controlModal="delSomeModal" :delCounts="delCounts" @delCancel="delSomeModal=false" @affirmDel="submitDelSome"></affirm-del-modal>
    </div>
    <!-- 删除多个弹窗 -->
    
    <!-- 删除1个弹窗 -->
    <div class="del-disease modal">
      <affirm-del-modal :controlModal="delOneModal" :delCounts="1" @delCancel="delOneModal=false" @affirmDel="submitDelOne"></affirm-del-modal>
    </div>
    <!-- 删除1个弹窗 -->
  </div>
</template>
<script>
  import { mapState, mapActions, mapMutations } from 'vuex'
  import fifters from '@/utils/fifters'
  import * as types from '@/store/modules/information/examManagement/examCategory/examCategoryTypes'
  import api from '@/api'
  import exportXlsx from '@/service/export-xlsx.js'
  export default {
    name: 'examCategory',
    data() {
      return {
        exportParams: {
          reqParams: {},
          columns: [
            {
              key: 'name',
              title: '体检类别名称',
            },
            {
              key: 'pe_type',
              title: '体检类型',
              render: (value) => {
                if (value === 1) {
                  return '健康体检类型'
                } else if (value === 2) {
                  return '职业体检类型'
                } else if (value === 3) {
                  return '放射体检类型'
                }
              },
            },
            {
              key: 'is_print',
              title: '是否打印证明',
              render: (value) => {
                return value === 1 ? '是' : '否'
              },
            },
            {
              key: 'is_upload',
              title: '是否上传',
              render: (value) => {
                return value === 1 ? '是' : '否'
              },
            },
            {
              title: '备注',
              key: 'remark',
            },
            {
              title: '启用',
              key: 'status',
              render: (value) => {
                return value === 1 ? '启用' : '未启用'
              },
            },
            {
              title: '显示顺序',
              key: 'sort',
            },
            {
              title: '最后修改人',
              key: 'operater',
            },
            {
              title: '最后修改时间',
              key: 'updated_at',
              render: (value) => {
                if (value) {
                  return fifters.timeFilter(value, 'YYYY-MM-DD HH:mm')
                } else {
                  return ''
                }
              },
            },
          ],
        },
        delSomeModal: false,
        delOneModal: false,
        selection: [],
        curRow: undefined,
        formAdd: {
          pe_type: '',
          name: '',
          remark: '',
          status: '0',
          is_print: '0',
          is_upload: '0',
          sort: 0,
        },
        itemClassList: [
          {
            label: '检验体检',
            value: 1,
          },
          {
            label: '检查体检',
            value: 2,
          },
          {
            label: '功能体检',
            value: 3,
          },
        ],
        pe_typeList: [
          {
            label: '健康体检类型',
            value: 1,
          },
          {
            label: '职业体检类型',
            value: 2,
          },
          {
            label: '放射体检类型',
            value: 3,
          },
        ],
        rulesFormAdd: {
          name: [
            {
              validator: (rule, value, callback) => {
                if (value !== null && (value + '').length > 0) {
                  callback()
                } else {
                  callback(new Error('体检类别名称不能为空'))
                }
              },
              trigger: 'blur',
            },
          ],
          pe_type: [
            {
              validator: (rule, value, callback) => {
                if (typeof value === 'number') {
                  callback()
                } else {
                  callback(new Error('体检类型不能为空'))
                }
              },
            },
          ],
        },
        urlSuccess: '/static/icons/public/enabledState.svg',
        urlRemark: '/static/icons/public/remarkInCell.svg',
        columns: [
          {
            title: '体检类别名称',
            key: 'name',
            ellipsis: true,
          },
          {
            title: '体检类型',
            key: 'pe_type',
            ellipsis: true,
            render: (h, params) => {
              let val = params.row.pe_type
              if (val === 1) {
                return '健康体检类型'
              }
              if (val === 2) {
                return '职业体检类型'
              }
              if (val === 3) {
                return '放射体检类型'
              }
            },
            titleRender: (row) => {
              let val = row.pe_type
              if (val === 1) {
                return '健康体检类型'
              } else if (val === 2) {
                return '职业体检类型男'
              } else if (val === 3) {
                return '放射体检类型'
              }
            },
            filterType: 'type',
            filterOptions: [
              {
                label: '健康体检类型',
                value: 1,
              },
              {
                label: '职业体检类型',
                value: 2,
              },
              {
                label: '放射体检类型',
                value: 3,
              },
            ],
          },
          {
            title: '是否打印证明',
            key: 'is_print',
            align: 'center',
            ellipsis: 'true',
            width: 110,
            render: (h, params) => {
              if (params.row.is_print === 1) {
                return h('div', [
                  h('div', {
                    style: {
                      margin: '0 auto',
                      background: `url(${this.urlSuccess}) no-repeat`,
                      width: '16px',
                      height: '16px',
                    },
                  }),
                ])
              }
            },
            titleRender: () => {
              return ''
            },
            filterType: 'type',
            filterOptions: [
              {
                label: '打印',
                value: 1,
              },
              {
                label: '不打印',
                value: 0,
              },
            ],
          },
          {
            title: '是否上传',
            key: 'is_upload',
            align: 'center',
            ellipsis: 'true',
            width: 90,
            render: (h, params) => {
              if (params.row.is_upload === 1) {
                return h('div', [
                  h('div', {
                    style: {
                      margin: '0 auto',
                      background: `url(${this.urlSuccess}) no-repeat`,
                      width: '16px',
                      height: '16px',
                    },
                  }),
                ])
              }
            },
            titleRender: () => {
              return ''
            },
            filterType: 'type',
            filterOptions: [
              {
                label: '上传',
                value: 1,
              },
              {
                label: '不上传',
                value: 0,
              },
            ],
          },
          {
            title: '备注',
            key: 'remark',
            ellipsis: true,
            width: 60,
            render: (h, params) => {
              let val = params.row.remark
              if (val) {
                return h(
                  'remark-in-table',
                  {
                    props: {
                      text: val,
                    },
                  }
                )
              }
              return ''
            },
          },
          {
            width: 60,
            title: '启用',
            key: 'status',
            align: 'center',
            ellipsis: 'true',
            render: (h, params) => {
              if (params.row.status === 1) {
                return h('status-in-table')
              }
            },
            titleRender: () => {
              return ''
            },
            filterType: 'type',
            filterOptions: [
              {
                value: 1,
                label: '启用',
              },
              {
                value: 0,
                label: '未启用',
              },
            ],
          },
          {
            title: '显示顺序',
            key: 'sort',
            align: 'center',
            ellipsis: 'true',
            width: 70,
          },
          {
            title: '最后修改人',
            key: 'operater',
            align: 'center',
            ellipsis: 'true',
            width: 90,
          },
          {
            title: '最后修改时间',
            key: 'updated_at',
            align: 'center',
            ellipsis: 'true',
            width: 130,
            render: (h, params) => {
              if (params.row.updated_at) {
                return fifters.timeFilter(params.row.updated_at, 'YYYY-MM-DD HH:mm')
              } else {
                return ''
              }
            },
            titleRender: (data) => {
              if (data.updated_at) {
                return fifters.timeFilter(
                  data.updated_at,
                  'YYYY-MM-DD HH:mm'
                )
              } else {
                return ''
              }
            },
            filterType: 'time',
          },
        ],
        statusList: [
          {
            label: '全部',
            value: 2,
          },
          {
            label: '启用',
            value: 1,
          },
          {
            label: '未启用',
            value: 0,
          },
        ],
        params: {
          search: '',
        },
        status: 2,
        modelDelete: false,
        rowIdArray: [],
        delCounts: 0,
        editModal: false,
        addModal: false,
        selectMode: false,
        tableHeight: 400,
      }
    },
    computed: {
      ...mapState(['windowHeight']),
      ...mapState('examCategoryModule', ['examCategoryList', 'total']),
      pageInfo() {
        return this.examCategoryList.meta
      },
      updated_at() {
        if (this.curRow && this.curRow.updated_at) {
          return fifters.timeFilter(this.curRow.updated_at, 'YYYY-MM-DD HH:mm')
        } else {
          return ''
        }
      },
    },
    methods: {
      // 验证输入框是否为空显示提示信息
      isEmpty(value) {
        if (value === '' || (value + '').trim() === '') {
          return true
        }
        return false
      },
      ...mapMutations('examCategoryModule', {
        reset: types.RESET,
        setTotal: types.SET_TOTAL,
      }),
      ...mapActions('examCategoryModule', [
        'getExamCategoryList',
        'addExamCategoryList',
        'updateExamCategoryList',
        'delExamCategoryList',
        'getTotal',
      ]),
      // ---------------搜索对话框按钮--------------
      search(data) {
        console.log(data)
        this.params.search = data
      },
      // ---------------新增对话框按钮--------------
      add() {
        this.formAdd = {
          name: '',
          pe_type: 1,
          remark: '',
          is_print: '0',
          is_upload: '0',
          status: '1',
          sort: this.total + 1,
        }
        this.addModal = true
      },
      async submitAdd() {
        // 新增对话框确定按钮
        let val = await this.$refs.formAdd.validate()
        if (val) {
          await this.addExamCategoryList(this.formAdd)
          this.$refs.zBaseInfoTable.request()
          this.addModal = false
          this.setTotal(this.total + 1)
        }
      },
      async submitEdit() {
        // 新增对话框确定按钮
        let val = await this.$refs.formCustomEdit.validate()
        if (val) {
          await this.updateExamCategoryList({
            id: this.curRow.id,
            data: this.formAdd,
          })
          this.$refs.zBaseInfoTable.request()
          this.editModal = false
        }
      },
      editOne(data, index) {
        this.curRow = data
        this.formAdd = {
          sort: data.sort,
          pe_type: data.pe_type,
          name: data.name,
          remark: data.remark,
          status: data.status + '' === '1' ? '1' : '0',
          is_print: data.is_print + '' === '1' ? '1' : '0',
          is_upload: data.is_upload + '' === '1' ? '1' : '0',
        }
        this.editModal = true
      },
      selectChange(selection) {
        this.selection = selection
      },
      deleteAllSelect() {
        this.delSomeModal = true
        this.delCounts = this.selection.length
      },
      delOne(data, index) {
        this.curRow = data
        this.delOneModal = true
      },
      print() {
        // console.log('打印')
      },
      exportBtn() {
        // console.log('导出')
        let _this = this
        exportXlsx('体检类别列表', {
          api: api.examCategory.getExamCategoryList,
          params: this.exportParams.reqParams,
          columns: this.exportParams.columns,
          callback (result) {
            if (result) {
              _this.$Message.success('导出成功')
            } else {
              _this.$Message.error('导出失败')
            }
          },
        })
      },
      getRequestParams(params) {
        this.exportParams.reqParams = params
      },
      getSort(value) {
        this.formAdd.sort = value
      },
      afterRequest() {
        // this.selection = []
      },
      async submitDelSome() {
        this.delSomeModal = false
        let arr = []
        this.selection.forEach(element => {
          arr.push(element.id)
        })
        await this.delExamCategoryList({ ids: arr })
        this.refreshAfterDel(this.selection.length)
      },
      async submitDelOne() {
        this.delOneModal = false
        await this.delExamCategoryList({ ids: this.curRow.id })
        this.refreshAfterDel(1)
      },
      refreshAfterDel(delNum) {
        this.$refs.zBaseInfoTable.allSelection = []
        this.selection = []
        this.setTotal(this.total - delNum)
        if (
          this.pageInfo.total <=
            (this.pageInfo.current_page - 1) * this.pageInfo.per_page + delNum &&
          this.pageInfo.current_page !== 1
        ) {
          this.$refs.zBaseInfoTable.request({ page: this.pageInfo.current_page - 1 })
        } else {
          this.$refs.zBaseInfoTable.request()
        }
      },
    },
    watch: {
      windowHeight() {
        this.tableHeight = this.$refs.contentTable.offsetHeight
      },
      status(data) {
        if (data === 2) {
          this.$set(this.params, 'status', '')
        } else {
          this.$set(this.params, 'status', data)
        }
      },
    },
    created() {
      this.getTotal()
    },
    mounted() {
      this.tableHeight = this.$refs.contentTable.offsetHeight
    },
    destroyed() {
      this.reset()
    },
    activated() {
      this.tableHeight = this.$refs.contentTable.offsetHeight
    },
  }
</script>
<style lang="less">
  @import (reference) '~assets/less/variable.less';
  .exam-category-wrapper {
    position: relative;
    margin: auto;
    height: 100%;

    .exam-category {
      height: 100%;
      width: 100%;
      padding: 10px;

      .header-wrapper {
        width: 100%;
        height: 100%;
        margin: 0 auto;
        border-radius: 8px;
        position: relative;
        background: #ffffff;
        border: 1px solid #d9d9d9;
        color: #111111;
        box-shadow: 0 2px 15px 0 rgba(206, 213, 242, 0.5);
        overflow: hidden;
        .contently {
          height: calc(~'100% - 50px');
        }
      }

      .table-header {
        position: relative;
        .titlet-slot-left {
          .start-state {
            line-height: 30px;
            font-size: 12px;
            color: @gradeAshThree;
            margin-right: 10px;
          }
        }
      }
    }

    .add-modal {
      .body {
        width: 100%;
        form {
          padding: 20px;
        }

        .lineH30 {
          height: 30px;
          line-height: 30px;
        }

        .check-group-row {
          .item .ivu-checkbox-wrapper {
            margin-right: 0;
          }
        }

        .row {
          & > label {
            display: table-cell;
            white-space: nowrap;
            width: 1%;
            color: @tagGroupText;
          }

          .require {
            position: absolute;
            right: 34px;
            top: 0;
            bottom: 0;
            line-height: 75px;
            font-size: 16px;
            color: #ff6262;
          }

          & .ls24 {
            padding-top: 10px;
            vertical-align: top;
            letter-spacing: 24px;
          }

          & .ls24:after {
            content: '';
            margin-left: -24px;
          }

          .time {
            margin-top: 15px;
            font-size: 12px;
            color: @tagGroupText;
            position: relative;

            span:first-child {
              margin-left: 20px;
            }

            span:nth-child(2) {
              position: absolute;
              top: 0;
              right: 20px;
            }
          }
        }
      }

      .inputRequired:after {
        content: '*';
        color: red;
        position: absolute;
        right: 12px;
        top: 7px;
        font-size: 16px;
      }

      .selectionRequired {
        .ivu-select-selection {
          position: relative;
        }
        .ivu-select-selection:after {
          content: '*';
          color: red;
          position: absolute;
          right: 20px;
          top: 2px;
          font-size: 16px;
        }
      }

      .clearfix:after {
        clear: both;
        content: '';
        display: block;
      }

      .statusCheckbox {
        margin-top: 7px;
        margin-right: 20px;
      }

      .lastOperater {
        line-height: 12px;
        padding-top: 20px;
        height: 32px;
      }
    }
  }
</style>
