    ...
    <div class="project-category">
      <div class="header-wrapper">
        <div class="eap-header">
          <title-has-slot>
            <span slot="titletHasSlotLeftTitle">照射源职业分类列表</span>
            <div slot="titletHasSlotRightContent" class="titlet-slot-left">
              <Button type="ghost" v-if="selectMode" class="fr " @click="selectMode=false"  key="3">取 消</Button>
              ...
              <!-- <Select v-model="proType" class="fr marginR20" style="width:92px " placeholder="全部" :filterable="true">
                <Option v-for="item in proTypeList" :value="item.value" :key="item.id">{{ item.label }}</Option>
              </Select>
              <span class="start-state fr">照射源</span> -->
            </div>
          </title-has-slot>
        </div>
        <div class="contently" ref="contentTable">
          <q-table
            ...
            :data="illuminationSourcesList.items"
      ...
      <Modal
        ...
        <div class="add-inquiry-center">
          ...
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;">照射源</span>
            <FormItem prop="source" class="fr">
              <Select
                filterable
                v-model="formCustomAdd.source"
                style="width:256px"
                placeholder=""
                :class="{selectionRequired:isEmpty(formCustomAdd.source)}">
                <Option v-for="item in itemClassList" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>
            </FormItem>
          </div>
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;">职业分类名称</span>
      ...
      <Modal
        ...
        <div class="add-inquiry-center">
          ...
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;">职业分类名称</span>
            <FormItem prop="name" class="fr">
              <Input style="width:256px" :class='{inputRequired: isEmpty(formCustomAdd.name)}' v-model.trim="formCustomAdd.name" :disabled="isstdDisabled" ></Input>
            </FormItem>
          </div>
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;margin-top:3px;">备注</span>
          ...
          <div class="row clearfix lastOperater" v-if="standard">
        ...
        <div slot="footer" class="clearfix">
          ...
          <Button class="fr marginR20 marginB20" v-if="standard" @click="submitEdit()">保 存</Button>
        </div>
      </Modal>
    </div>
    <!-- 编辑结束-->
...
<script>
  ...
  import * as types from '@/store/modules/information/parameterManagement/illuminationSources/illuminationSourcesTypes'
  import api from '@/api'
  ...
  export default {
    ...
    data() {
      return {
        ...
        proType: 0,
        modelDelete: false,
        headerTexadd: '新增照射源职业分类',
        headerTexComplite: '编辑照射源职业分类',
        rowIdArray: [],
        delCounts: 0,
        compileMode: false,
        addMode: false,
    ...
    computed: {
      ...
      ...mapState('illuminationSourcesModule', ['illuminationSourcesList', 'total']),
      pageInfo() {
        // console.log(this.illuminationSourcesList.meta)
        return this.illuminationSourcesList.meta
      },
      updated_at() {
    ...
    methods: {
      ...
      ...mapMutations('illuminationSourcesModule', {
        reset: types.RESET,
        setTotal: types.SET_TOTAL,
      }),
      ...mapActions('illuminationSourcesModule', [
        'getSamplingState',
        'createSamplingState',
        'deleteSampleState',
        'writeSampleState',
      ...
      listEditRowStandard(data) {
        if (!data.is_std) {
          this.isstdDisabled = false
        } else {
          this.isstdDisabled = true
        }
      },
      // ---------------搜索对话框按钮--------------
      search(data) {
      ...
      exportBtn() {
        // console.log('导出')
        let _this = this
        exportXlsx('照射源职业分类管理', {
          api: api.illuminationSources.state,
      ...
      async submitAdd() {
        ...
        if (val) {
          await this.createSamplingState(this.formCustomAdd)
      ...
      async submitDelSome() {
        ...
        await this.deleteSampleState({ ids: arr })
        this.refreshAfterDel(this.selection.length)
      },
      async submitDelOne() {
        this.delOneModal = false
        await this.deleteSampleState({ ids: this.curRow.id })
    ...
    watch: {
      windowHeight() {
        ...
        // this.$refs.zBaseInfoTable.handleResize()
      },
      // status(data) {
      //   if (data === 2) {
      ...
      proType(data) {
        console.log(88, data)
        if (data === 0) {
          this.paraming.filters = 'source:'
        } else {
          this.paraming.filters = 'source:' + data
        }
      },
    },
    created() {
