...
*/
<template>
  <div class="teamManagement-category-wrapper">
    <!-- 主体 -->
    <div class="teamManagement-category">
      <div class="header-wrapper">
        <div class="eap-header">
          <title-has-slot>
            <span slot="titletHasSlotLeftTitle">排队机列表</span>
      ...
      <Modal
        ...
        <div class="add-inquiry-center">
          ...
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;">排队机编号</span>
            <FormItem prop="code" class="fr">
              <Input style="width:351px" :class='{inputRequired: isEmpty(formCustomAdd.code)}' v-model.trim="formCustomAdd.code"></Input>
            </FormItem>
          </div>
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;">排队机名称</span>
            <FormItem prop="name" class="fr">
              <Input style="width:351px" :class='{inputRequired: isEmpty(formCustomAdd.name)}' v-model.trim="formCustomAdd.name"></Input>
            </FormItem>
          </div>
          <div class="row clearfix">
            <span class="textJustify fl lineH30" style="width:72px;margin-top:3px;">备注</span>
            <FormItem class="fr">
              <Input style="width:351px" v-model="formCustomAdd.remark" type="textarea" :row="2"></Input>
            </FormItem>
          </div>
          </Form>
        </div>
...
<script>
  ...
  import * as types from '@/store/modules/information/manageOffice/teamManagement/teamManagementTypes'
  import api from '@/api'
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState('teamManagementModule', ['diseaseTypesList', 'total']),
      pageInfo() {
    ...
    methods: {
      ...
      ...mapMutations('teamManagementModule', {
        reset: types.RESET,
        setTotal: types.SET_TOTAL,
      }),
      ...mapActions('teamManagementModule', [
        'getDiseaseTypesFunc',
        'addDiseaseTypesFunc',
      ...
      exportBtn() {
        ...
        let _this = this
        exportXlsx('排队机管理', {
          api: api.teamManagement.getDiseaseTypesFunc,
...
<style lang="less">
  ...
  .teamManagement-category-wrapper {
    position: relative;
    margin: auto;
    height: 100%;

    .teamManagement-category {
      height: 100%;
      width: 100%;
      padding: 10px;
