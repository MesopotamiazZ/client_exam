<template>
  <div>
    ...
    <Modal v-model="show"
           ...
           :closable="false">
      <div slot="header">
        <alert-header headerText="编辑诊断规则"
      ...
      <Form :model="formData"
        ...
        <div class="drm-form-row">
          <div class="drm-row-item drm-item-small">
            ...
            <FormItem class="marginL10"
                      style="width:146px;" prop="item_type_id">
                      <!-- TODO: -->
              <Select v-model="formData.item_type_id" filterable placeholder=" " v-if="itemTypes.items.length">
        ...
        <div class="drm-form-row">
          <div class="drm-row-item drm-item-small">
            ...
            <FormItem class="marginL10"
                      style="width:146px;" prop="diagnose_result">
              <Select v-model="formData.diagnose_result.id" filterable placeholder=" " v-if="result.items.length">
                <Option v-for="item in result.items" :value="item.id":key="item.id">{{item.name}}</Option>
              </Select>
            </FormItem>
          </div>
          <div class="drm-row-item drm-item-large marginL20">
            ...
            <FormItem class="marginL10"
                      style="width:64px;" prop="sex">
              <Select v-model="formData.sex" placeholder=" ">
            ...
            <FormItem class="marginL10"
                      style="width:40px;">
              <Input v-model="formData.age_x" number placeholder=" "></Input>
            </FormItem>
            <div class="drm-flex-line"></div>
            <FormItem style="width:40px;" prop="age_y">
              <Input v-model="formData.age_y" number placeholder=" "></Input>
            </FormItem>
          </div>
        </div>
        ...
        <div class="drm-form-textarea marginB20">
          ...
          <FormItem class="drm-flex1 marginL10">
            <Input type="textarea"
                   :autosize="{minRows: 1,maxRows: 2}" v-model="formData.remark"></Input>
          </FormItem>
        </div>
      </Form>
      <div slot="footer">
    ...
    <card-modal ref="cardModal"
                :show="showCard"
                @visibleChange="cardVisibleChange" @on-change="cardChange"></card-modal>
    <!-- <Modal
      v-model="showCard"
      :transition-names='[]'
      :mask-closable="false"
...
export default {
  ...
  data() {
    ...
    const ageValidator = (rule, value, callback, source, options) => {
      ...
      if (this.formData.age_x.toString().length > 0 && value.toString().length <= 0) {
          ...
          render: (h, params) => {
            ...
            return h(
              'Select',
              {
                props: {
                  placeholder: ' ',
                  value: this.formData.rules[params.index].prep_bracket.toString(),
                },
                on: {
                  'on-change': (event) => {
        ...
        // },
        {
          ...
          render: (h, params) => {
            return h('Select', {
              props: {
                placeholder: ' ',
                value: this.formData.rules[params.index].operator.toString(),
              },
              on: {
                'on-change': (event) => {
                  // console.log('失1焦', event)
  ...
  methods: {
    ...
    cardChange(val, type, label) {
      // TODO:
      // console.log('阿萨德飞机离开', this.selectionRow)
    ...
    cleanSelectionRule() {
      // console.log('选中行', this.selectionRow)
