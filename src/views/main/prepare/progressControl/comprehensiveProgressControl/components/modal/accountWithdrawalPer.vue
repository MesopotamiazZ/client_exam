...
* @author 覃凤
* @date 2018/03/7
* @content 账户提现（修改单位管理页面）
*/ 
<template>
  <div class="account-withdrawalper">
      ...
      <div class="main">
        <Form ref="withdrawalForm"
          ...
          <FormItem label="提现金额"
                    ...
                    class="withdrawal-form-row three">
            <z-input type="number" v-model.trim="withdrawalForm.data.withdrawalAmount" class="yuan"></z-input>
          </FormItem>
          <FormItem label="提现原因"
                    prop="reason"
...
export default {
  ...
  data() {
    return {
      withdrawalForm: {
        data: {
          unit: '四川小蓝科技', // 单位名称
          accountId: '254948774', // 账户编号
          accountBlance: '25.00', // 账户余额
  ...
  methods: {
    affirm () {
      this.$emit('commit-data', false)
    },
    cancel () {
      this.$emit('cancel-data', false)
    },
    initClear () {
