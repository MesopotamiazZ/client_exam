  ...
  <div class="choose-items-modal-surcharge container">
    <Form ref="surchargeForm"
      ...
      <div class="row"
        ...
        <FormItem label="费用类别"
                  :label-width="70"
                  :prop="`items.${index}.charge_category_id`"
                  :rules="changeSurchargeRules">
          <Select v-model="item.charge_category_id"
                  :disabled="item.status !== 0"
                  filterable>
            <Option v-for="(cc, i) in chargeCategories"
                    :value="cc.id"
                    :key="cc.id"
                    @click.native.stop="setSurchargeOptions(item, cc)">{{cc.name}}</option>
          </Select>
        </FormItem>
        <FormItem label="附加费名称"
          ...
          <template v-if="item.surchargeOptions.length">
            <Select v-model="item.surcharge_id"
                    :disabled="item.status !== 0"
                    filterable>
      ...
      <div class="row">
        <FormItem label="费用类别"
                  :label-width="70"
                  prop="default.charge_category_id">
          <Select v-model="surchargeForm.default.charge_category_id"
                  ref="defaultSelect"
                  filterable>
            <Option v-for="(cc, i) in chargeCategories"
                    :value="cc.id"
                    :key="i"
                    @click.native.stop="setSurchargeOptions(surchargeForm.default, cc)">{{cc.name}}</option>
          </Select>
        </FormItem>
        <FormItem label="附加费名称"
                  :label-width="80"
...
export default {
  ...
  data() {
    return {
        ...
        },
        {
          validator: (rule, value, callback) => {
            if (
              ...
              !this.surchargeForm.default.charge_category_id ||
      ...
      defaultSurchargeRules: [
        {
          validator: (rule, value, callback) => {
            if (value || !this.surchargeForm.default.charge_category_id) {
              callback()
            } else {
              callback('附加费名称必须选择')
            }
  ...
  methods: {
    ...
    getNewItem(item) {
      return {
        ...
        charge_category_name: item.charge_category_name,
    ...
    checkItem(item) {
      if (
        typeof item.charge_category_id === 'number' &&
    ...
    setSurchargeOptions(item, chargeCategory = {}) {
      item.surchargeOptions = chargeCategory.surcharges || []
      item.charge_category_name = chargeCategory.name
    },
  ...
  computed: {
    ...
    canAddItem() {
      ...
      if (
        defaultItem.charge_category_id &&
