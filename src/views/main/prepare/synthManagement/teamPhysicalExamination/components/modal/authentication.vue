...
<template>
  <div class="authentication"
    ...
    <Modal v-model="authenticationModal"
           :styles="{width: '356px'}"
           :transition-names='[]'
           :mask-closable="false"
           :transfer='false'
      ...
      <div slot="header">
        <div slot="header">
          <div name='alertHeader'
            ...
            <span class="alert-header-text font16 fl">认证</span>
          </div>
        </div>
      </div>
      <div class="reset-phone-main">
        <Form ref='editData'
              :model="editData"
              :rules="editRule">
          <div class="left-name">验证手机号</div>
          <FormItem style="display:inline-block" prop="mobile">
            <Input v-model.trim="editData.mobile"
                   class="right-input"
                   :class='{inputVerificationTs: isEmpty(editData.mobile)}'
                   style="width: 234px;" />
          </FormItem>
          ...
          <div class="left-name">确认支付密码</div>
...
export default {
  ...
  data() {
    const validatePhone = (rule, value, callback) => {
      ...
      } else if (!phoneTest.test(numValue)) {
