<!--
*单位管理-单位账户详情-修改单位支付密码
*@author 刘佳莉
*@date 2018/03/02
-->
<template>
  <div class="edit-unit-password"
       v-if="editUnitPayModal">
    <Modal v-model="editUnitPayModal"
           :styles="{width: '356px'}"
           :transition-names='[]'
           :mask-closable="false"
           :transfer='false'
      ...
      <div slot="header">
        <div slot="header">
          <div name='alertHeader'
            ...
            <span class="alert-header-text font16 fl">修改单位支付密码</span>
            <div class="alert-header-input fr">
              <span class="curPointer font12" @click="resetPsd">重置密码</span>
            </div>
          </div>
        </div>
      </div>
      <div class="reset-phone-main">
        <Form ref='editData'
          ...
          <div class="left-name">旧密码</div>
          <FormItem prop="oldPassword"
                    style="display:inline-block">
            <Input v-model.trim="editData.oldPassword"
                   type="password"
                   class="right-input"
                   :class='{inputVerificationTs: isEmpty(editData.oldPassword)}'
                   style="width: 258px;" />
          </FormItem>
