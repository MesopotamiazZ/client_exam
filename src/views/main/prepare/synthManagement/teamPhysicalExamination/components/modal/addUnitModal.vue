...
<template>
  <Modal v-model="addUnitModal"
         ...
         :closable="false">
    <div slot="header">
      <alert-header :showInput='false' headerText="新增单位">
      </alert-header>
    </div>
    ...
    <div slot="footer">
      <div class="bottom">
        <div class="bottom-right">
          <Button type="primary"
                  class="button"
                  @click="saveAndEdit">保存并编辑车间/部门</Button>
          <Button type="primary"
                  class="button"
    ...
    <!-- 新增危害 -->
    <addOrModifyOccupHarmFactor :controlModal='controlModal'
                                @add-affirm-factor='addFactorSave'
                                @cancel-add-factor='addFactorCancel'></addOrModifyOccupHarmFactor>
...
export default {
  ...
  methods: {
    ...
    getData(data) {
      console.log('jiangyiling', data)
    },
    sure() {
      this.$emit('add-unit-save')
    },
    cancel() {
      this.$emit('add-unit-cancel')
    },
    saveAndEdit() {
      this.$emit('add-unit-save-and-edit')
    },
    // 新增
    add() {
