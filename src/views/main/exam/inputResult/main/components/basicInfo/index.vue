<template>
  <div class="input-basicInformationOuterBox">
    <div class="basicInfo">
      <div class="schema">
      ...
      <div class="infos">
        ...
        <template v-if="examType === 3">
          ...
          <div class="line4 line radiation">
            <span class="info-item" style="flex-grow: 1; width: 211px"><label>目标疾病</label><span class="item-value" style="width: calc(100% - 58px);">{{targetDisease}}</span></span>
      ...
      <div class="quoteGbz">
        ...
        <div class="content" v-show="gbzData.show" v-html="gbzData.queue[gbzData.queue.length - 1]"></div>
...
//      console.log('do query ', examinationNumber)
      // TODO1804010126
      projectGetBasic(examinationNumber).then((data) => {
//        console.log(data, '--=-=')
        this.recheck = this.getter(data, 'is_recheck')
        this.photo = this.getter(data, 'customer.photo')
        this.userName = this.getter(data, 'customer.realname')
        this.sendBack = this.getter(data, 'is_lock')
        this.gender = this.getter(data, 'customer.sex')
        this.age = this.getter(data, 'customer.age')
        this.examType = this.getter(data, 'pe_type')
        this.company = this.getter(data, 'unit_appt.unit.name')
        this.workCategory = this.getter(data, 'occup_info.occup_type_name')
        this.touchingYears = this.getter(data, 'occup_harm_factors[0].year')
        this.injuryFactor = this.getter(data, 'occup_harm_factors[0].name')
        this.serverData = data
        this.gbzData.key = this.getter(data, 'occup_harm_factors[0].gbzKey')
...
//    })
  ...
  beforeDestroy() {
    this.$el.desubscribeScanGun(this.aaa)
  },
}
...
<style lang="less" >
  .input-basicInformationOuterBox{
    /*display: flex;*/
    position: relative;
    width: 100%;
