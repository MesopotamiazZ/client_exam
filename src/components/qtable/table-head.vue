  ...
  <table cellspacing="0"
    ...
    <colgroup ref="colgroup">
      <col v-for="(column, index) in columns"
           :width="setCellWidth(column, index, true, data.length)">
    </colgroup>
    <thead>
      <tr>
        <th v-for="(column, index) in columns"
          ...
          <div :class="cellClasses(column)"
            ...
            <template v-else-if="column.type === 'selection'">
              <Checkbox :value="isSelectAll"
                        @on-change="selectAll"></Checkbox>
            </template>
            <template v-else>
              <span v-if="!column.renderHeader"
                    @click="handleSortByHead(index)">{{ column.title || '#' }}</span>
              <render-header v-else
                             :render="column.renderHeader"
                             :column="column"
                             :index="index"></render-header>
              <span :class="[prefixCls + '-sort']"
                ...
                <div slot="content"
                  ...
                  <div :class="[prefixCls + '-filter-footer']">
                    <i-button type="text"
                              ...
                              @click.native="handleFilter(index)">{{ t('i.table.confirmFilter') }}</i-button>
                    <i-button type="text"
                              size="small"
                              @click.native="handleReset(index)">{{ t('i.table.resetFilter') }}</i-button>
                  </div>
                </div>
                <div slot="content"
                     :class="[prefixCls + '-filter-list']"
                  ...
                  <ul :class="[prefixCls + '-filter-list-single']">
                    <li :class="itemAllClasses(column)"
