<template>
  <div :class="[wrapClasses, {resizing: colResizing}]"
       :style="styles"
       class="qtable_outerBox"
       ref="qtable">
    <div :class="classes">
      ...
      <div :class="[prefixCls + '-header']"
        ...
        <table-head ref="tableHeader"
                    ...
                    :fixedRightWidth="fixedRightWidth"></table-head>
      </div>
      <div :class="[prefixCls + '-body']"
        ...
        <table-body ref="tbody"
                    ...
                    :columns-width="columnsWidth"
                    :obj-data="objData"
                    :style="{minWidth: (hasScrollbar ? tableContentWidth - scrollBarWidth : tableContentWidth)+'px'}"
                    :fixedRightWidth="fixedRightWidth"
                    @buttonRight="buttonRight"></table-body>
      </div>
      ...
      <div :class="[prefixCls + '-fixed-right']"
        ...
        <div :class="fixedHeaderClasses"
             v-if="showHeader">
        <div v-if="hasScrollbar" class="mend" style="display: inline-block; position: absolute; width: 6px; height: 40px; background: rgb(215, 233, 255); right: -6px;"></div>
        ...
        <div :class="[prefixCls + '-fixed-body']"
          ...
          <table-body fixed="right"
                      ...
                      :fixedRightWidth="fixedRightWidth"></table-body>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      fontWidthInitialed: false,
      hasScrollbar: false,
      relevantContainerWidth: 0,
    }
  },
  computed: {
    localeNoDataText() {
    ...
    fixedBodyStyle() {
      // 由于非固定的表格要撑开table区域把滚动条显示在最底部,因此bodystyle的高度不能设置为实际的高度,
      let style = {}

      if (this.bodyHeight > 0) {
        // 设置侧边固定表格长度为表格整体宽度 判断表格主体是否存在滚动条 不存在滚动条就加上滚动条的宽度
        let height = this.bodyHeight
        if (parseInt(getStyle(this.$refs.tbody.$el, 'width')) <= this.tableWidth) {
          height += this.scrollBarWidth - 1
        }
        ...
        let tableFixedRight =
          this.$refs.qtable &&
          this.$refs.qtable.querySelector('.ivu-table-fixed-right') // document.querySelector('.ivu-table-fixed-right')

        if (height < this.bodyRealHeight) {
          // console.log(
          ...
          // }
          // 在不影响固定列的跳动,有滚动条的时候, 固定列宽要减去滚动条宽度 -- 暂时写死(76-6)px
          tableFixedRight.style.right = `${this.scrollBarWidth}px`
          tableFixedRight.style.width = `${this.fixedRightWidth}px`
          this.hasScrollbar = true
        } else {
          // 当body实际高度小于表格显示高度,固定列高度为实际高度
          // 此时没有纵向滚动条,固定列不偏移6px
          // 在不影响固定列的跳动,没有滚动条的时候, 固定列宽为原始的 -- 暂时写死76px
          height = this.bodyRealHeight
          tableFixedRight.style.right = 0
          tableFixedRight.style.width = `${this.fixedRightWidth}px`
        ...
        // 不知道为什么要写这个else, yyyyy???
        // let tableFixedRight = document.querySelector('.ivu-table-fixed-right')
        let tableFixedRight =
          this.$refs.qtable &&
          this.$refs.qtable.querySelector('.ivu-table-fixed-right')
        if (tableFixedRight) {
          tableFixedRight.style.right = 0
        }
      }
  ...
  methods: {
    buttonRight(data, event, index) {
      this.$emit('button-right', data, event, index)
    },
    rowClsName(index) {
    ...
    toggleSelect(_index, b) {
      if (b !== 1) {
        ...
        this.$emit('on-selection-value', status, data)
      } else {
        let data = {}
        for (let i in this.objData) {
          if (parseInt(i) === _index) {
    ...
    selectAll(status) {
      ...
      this.$emit('on-selection-change', selection)
      this.$emit('on-selection-all-value', status, selection)
    },
    fixedHeader() {
      if (this.height) {
        this.$nextTick(() => {
          ...
          this.bodyHeight = this.height - 40
        })
      } else {
        this.bodyHeight = 0
      }
    ...
    makeColumns() {
      ...
      // console.log('-=-==-', left.concat(center).concat(right))
...
.qtable_outerBox {
  position: relative;
  th,
  td:not(.ivu-table-expanded-cell) {
    & {
      overflow: hidden;
