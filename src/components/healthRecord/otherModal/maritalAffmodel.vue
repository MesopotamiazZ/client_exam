...
export default {
  ...
  methods: {
    // 打开莫泰框
    ...mapActions('healthRecordModule', ['marriagePost']),
    openM() {
      this.delModel = true
    },
    // 关闭莫泰框
    ...
    affirm() {
      let that = this
      let data = copy(this.transfusionArr, true)
      for (let key of data) {
        if (key.operater) {
          delete key.operater
        }
        ...
        //  key.wedding_date = key.wedding_date.getTime() / 1000 ? key.date.getTime() / 1000 : ''
      }
      data.forEach(element => {
        element.ha_id = this.id
        element.wedding_date = new Date(element.wedding_date) / 1000
      })
      this.marriagePost(data).then(v => {
        if (v) {
          that.$emit('refresh')
          that.closeM()
        }
