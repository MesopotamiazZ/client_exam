...
export default {
  ...
  methods: {
    // 打开莫泰框
    ...mapActions('healthRecordModule', ['geneticDiseasePost']),
    openM() {
      this.delModel = true
    },
    // 关闭莫泰框
    ...
    affirm() {
      let that = this
      let data = copy(this.transmit)
      let a = {}
      a.type = this.typeN
      a.ha_id = this.id
      for (let key in data) {
        if (data[key] === '其他') {
          data[key] = { name: data[key], remark: this.remark }
        } else {
          data[key] = { name: data[key] }
        }
      }
      a.value = data
      this.geneticDiseasePost(a).then(data => {
        if (data) {
          that.$emit('refresh')
          that.closeM()
        }
