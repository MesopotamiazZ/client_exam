    ...
    <Modal v-model="addModel"
      ...
      <div slot="header">
        <titleHasSlot style="border-bottom: 1px solid #cccccc;">
          <span slot='titletHasSlotLeftTitle'>编辑职业照射种类</span>
        </titleHasSlot>
      </div>
          ...
          <Row style="margin-top:15px">
            <div class='verification'
              ...
              <div class=" fontC textJustify"
                   style=" color:#797979;width:36px;">照射源</div>
              <div class="lageIput"
                   style="height:30px;width:308px;">
...
export default {
  ...
  methods: {
    ...
    diagnoseTypeChange(val) {
      ...
      this.irradiationGet(val).then(data => {
        if (data) {
    ...
    affirm() {
      ...
      this.$emit('checkValue', this.checkArr, this.diagnoseType)
    },
    abolish() {
      this.addModel = false
    },
