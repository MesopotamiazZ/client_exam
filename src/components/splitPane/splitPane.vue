...
<template>
    <div :class="wraperClasses" ref="wraper" @mouseup="handleMouseup" @mousemove="handleMousemove" @mouseleave="handleMouseout">
        <div v-if="direction === 'horizontal'" :class="`${prefix}-container`">
            ...
            <slot name="trigger">
                <div :class="`${prefix}-trigger`" ref="trigger" :style="{left: `${triggerOffset}%`}" @mousedown="handleMousedown" unselectable="on">
                </div>
            </slot>
        ...
        <div v-else :class="`${prefix}-container`">
            ...
            <slot name="trigger">
                <div :class="`${prefix}-trigger`" ref="trigger" :style="{top: `${triggerOffset}%`}" @mousedown="handleMousedown" unselectable="on">
                </div>
            </slot>
    ...
    handleMousemove(e) {
      if (this.canMove) {
        ...
        if (this.direction === 'horizontal') {
          offset =
            this.triggerOldOffset +
            Math.floor(
              (e.clientX - this.offset.x) /
                this.$refs.wraper.offsetWidth *
                10000
            ) /
              100
        } else {
