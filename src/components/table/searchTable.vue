        ...
        <ul
          ...
          <template v-for="(items, index) in newDataList"  v-if="ifShowSearchtitle">
            <li
              ...
              <div class="fl overflowEllipsis" v-for="(columns, index2) in newColumns" :key="index2">{{index2===0?index+1:items[columns.key]}}</div>
            </li>
          </template>
          <template v-for="(items, index) in newDataList"   v-if="!ifShowSearchtitle">
            <li
...
<script>
  import tableSearchTitle from './tableSearchTitle'
  ...
  export default {
    ...
    mounted() {
      console.log(this.columns, 'this.columnsthis.columnsthis.columnsthis.columns')
      // console.log('进来了', this.newDataList)
      this.initStyle()
      ...
      //      searchBody.addEventListener('scroll', this.scrollFun)
      // this.tableH = this.$refs.searchTableValH.offsetHeight // 表格数据高度
      // this.initHasScroll()
      // this.hasScrolled(this.$refs.searchTableH)

      //      let listItems = this.$refs.searchTableValH.children
      //      for (let i = 0; i < listItems.length; i++) {
      //        listItems[i].setAttribute('data-offsetTop', listItems[i].offsetTop)
      //      }
    },
    watch: {
      //      'scrollTopVal' (data) {
      //        this.iconPosition(this.clickIndex)
      //      },
      iconShow(data) {
        // console.log(data)
      },
