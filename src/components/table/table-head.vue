<template>
    <table cellspacing="0" cellpadding="0" border="0" :style="styles">
      <colgroup>
          ...
          <col v-for="(column, index) in columns" :style="setCellWidth(column, index, true, data.length)">
      </colgroup>
      <thead>
        <tr>
          <th v-if="isShowSelectOrIndex && fixed !== 'right'" v-show="isSelectMode" style="width: 50px;">
            <Checkbox :value="isSelectAll" @on-change="selectAll"></Checkbox>
          </th>
          <th v-for="column in indexColumn" v-if="isShowSelectOrIndex && fixed !== 'right' && !isSelectMode" style="width: 50px">
          ...
          <th
            ...
            :style="isDependent?{'border-right-width': '1px'}:setRightBorder(column,columns)">
            <div :class="cellClasses(column)">
...
<script>
  ...
  export default {
    ...
    created() {
      // console.log(this.isShowSelectOrIndex)
    },
  }
</script>
